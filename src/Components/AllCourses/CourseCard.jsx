...
export default function CourseCard({course}){
  const {course_name,course_image,category,level,price,duration,instructor,Purchase_order} = course;
  return (
    ...
    <div className="h-52 overflow-hidden">
      <div>
          ...
          <h3 className='absolute top-0 left-0 bg-TealGreen p-2 pr-5 pb-5 text-white rounded-br-full'>${price}</h3>
      </div>
    </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between">
       ...
       <div className="text-gray-500 text-sm lg:base">
        ...
        <p className='flex items-center gap-2'><HiUsers className="text-lg"/> {Purchase_order}</p>
        </div>
       </div>
